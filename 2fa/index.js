  ...
  if (!resultCell.value) {
    for (let i = 0; i < 2; i++) {
      ...
      try {
        ...
        await page.waitForSelector('input[id="confirm-disable-dialog-modal-submit"]',{
          timeout: 10000
        })
        await delay(1000)
        ...
        await page.$eval('input[id="remove-devices-checkbox-input"]', check => check.checked = true);
        await delay(1000)
        ...
        const clickApp = await page.waitForSelector('a[id=sia-otp-accordion-totp-header]', {
          timeout: 5000
        })
